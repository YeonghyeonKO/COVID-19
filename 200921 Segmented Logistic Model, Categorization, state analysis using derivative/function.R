...
cumulative_analysis <- function(Country, end_date=max_date, 
  ...
  for(i in Country){
    ...
    if(df_result$break_number[df_result$country==i]==1){
      ...
      for(j in 1:df_result$break_number[df_result$country==i]){
        x_start = df_result[df_result$country==i,12+j] - t
        x_end = ifelse(is.na(df_result[df_result$country==i,13+j]),max(x),df_result[df_result$country==i,13+j]) -t
        x_=x[x_start:x_end]
        y_=y[x_] - y[x_start-1]
        x_=x_-x_start
        st = data.frame(a = c(0.5*max(y_), max(y_)*30), b = c(0, 25), c = c(0, 0.5))
        
        fit11 = try(nls2(y_ ~ a/(1+exp(b-c*x_)+0), start = st,
...
segLogistic = function(Country,start_date=1,
  ...
  path <- "C:/Users/kyh/GitHub/COVID-19/200921 Segmented Logistic Model, Categorization, state analysis using derivative/"
  result_path <- paste0(path, "Result/")
  plot_path <- paste0(path, "Plot/")
  ...
  if(m > 1){
    X_design = cbind(x)
    for(j in 1:(m-1)){
      X_design = cbind(X_design, (-b[j+1] + 1):(n - b[j+1]))
    }
  }else{
    X_design = cbind(x)
  }
  ...
  tryCatch(
    expr = {
      ...
      if(m==1){
        pred_y = predict(fit12, newdata=X_design)
        r_square = 1 - sum((y - pred_y)^2) / sum((y - mean(y))^2)
        
        # plot
        p = ggplot()+
          geom_point(aes(x, y), alpha = 0.7, shape = 1) +
          geom_line(aes(x, pred_y), col = 2, size = 1) +
          labs(title = paste0("Segmented Logistic Model of ", Country),
               subtitle = paste0("Breakpoint : ",
                                 "\nR - Square : ", round(r_square, 4)),
               x = paste("Days since", as.Date("2019-12-31") + b[1]),
               y = "Cumulative confirmed cases", main=Country) +
          theme_bw() +
      ...
      if(m>=2){
        ...
        fit21 = try(
          nls2(y_ ~ a1/(1+exp(b1-c1*x1)) + a2/(1+exp(b2-c2*x2)),
               data=X_design_, start = st,
               algorithm = "plinear-random", control = nls.control(maxiter = 5000))
          )
        ...
        fit22 = try( 
          nls(y_ ~ a1/(1+exp(b1-c1*x1)) + a2/(1+exp(b2-c2*x2)),
              data=X_design_,start = initial_list) 
          )
        ...
        if(m==2){
          pred_y = predict(fit22, newdata=X_design)
          r_square = 1 - sum((y - pred_y)^2) / sum((y - mean(y))^2)
          
          p = ggplot()+
            geom_point(aes(x, y), alpha = 0.7, shape = 1) +
            geom_line(aes(x, pred_y), col = 2, size = 1) +
            geom_vline(aes(xintercept = b[2]-b[1]), linetype = 3, col = 3, size = 1) +
            labs(title = paste0("Segmented Logistic Model of ", Country),
                 subtitle = paste0("Breakpoint : ", b[2]-b[1],
                                   "\nR - Square : ", round(r_square, 4)),
                 x = paste("Days since", as.Date("2019-12-31") + b[1]),
                 y = "Cumulative confirmed cases", main=Country) +
            theme_bw() +
      ...
      if(m>=3){
        ...
        fit31 = try(
          nls2(y_ ~ a1/(1+exp(b1-c1*x1)) + a2/(1+exp(b2-c2*x2)) + a3/(1+exp(b3-c3*x3)),
               data=X_design_, start = st,
               algorithm = "plinear-random", control = nls.control(maxiter = 5000))
          )
        ...
        fit32 = try( 
          nls(y_ ~ a1/(1+exp(b1-c1*x1)) + a2/(1+exp(b2-c2*x2)) + a3/(1+exp(b3-c3*x3)), 
              data=X_design_, start = initial_list) 
          )
        ...
        if(m==3){
          pred_y = predict(fit32, newdata=X_design)
          r_square = 1 - sum((y - pred_y)^2) / sum((y - mean(y))^2)
          
          p = ggplot()+
            geom_point(aes(x, y), alpha = 0.7, shape = 1) +
            geom_line(aes(x, pred_y), col = 2, size = 1) +
            geom_vline(aes(xintercept = c(b[2]-b[1], b[3]-b[1])), linetype = 4, col = 3, size = 1) +
            labs(title = paste0("Segmented Logistic Model of ", Country),
                 subtitle = paste0("Breakpoint : ", b[2]-b[1], " / ", b[3]-b[1],
                                   "\nR - Square : ", round(r_square, 4)),
                 x = paste("Days since", as.Date("2019-12-31") + b[1]),
                 y = "Cumulative confirmed cases", main=Country) +
            theme_bw() +
      ...
      write.csv(coef_result, paste0(result_path, "Coef_Logistic_", Country, ".csv"), row.names = T)
      ggsave(p, filename = paste0("Segmented_Logistic_Model_of_", Country, ".png"), 
             path = plot_path, width = 12, height = 7, dpi = 300) 
    },
    error = function(e){
      cat("not fitted\n")
    }
  ...
  rownames(coef_result) = c(1, 2, 3, 4, 5)[1:m]
  cat(paste("-----  ", Country,"  -----\n"))
  print(coef_result); print(paste0("R-Square : ", r_square))
  return(coef_result)
