...
setwd("/Users/yeonghyeon/Documents/GitHub/COVID-19/201011 Segmented Growth Curve coefficients")
getwd()
...
df_result = derivative_analysis(Country = country,gkf_bandwidth = 14,first_break = 50,save_image = FALSE,save_excel = TRUE)
...
for(i in country){
  tryCatch(
    expr={
      ...
      coef_sep_Logi[i, 1:3] <- segLogistic_separate(Country=i,break_point = break_point,save_image = FALSE,prediction_plot = FALSE,
                  max_iter = 1000)[1,]
      coef_sep_Logi[i, 4:6] <- segLogistic_separate(Country=i,break_point = break_point,save_image = FALSE,prediction_plot = FALSE,
                                                    max_iter = 1000)[2,]  
...
write.csv(coef_sep_Logi, "Separated_Logistic_coefficients.csv")
...
a1 <- ggplot(scaled_common_coef_1) + 
  ...
  labs(title="Standardized Coefficients between Logistic vs Gompertz", x="", y="a1 coefficient",
       subtitle="1st Segmentation interval") +
...
a2 <- ggplot(scaled_common_coef_2) + 
  ...
  labs(title="Standardized Coefficients between Logistic vs Gompertz", x="", y="a1 coefficient",
       subtitle="2nd Segmentation interval") +
