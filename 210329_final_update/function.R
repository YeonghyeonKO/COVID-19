...
segLogistic = function(Country,start_date=1,
  ...
  plot(x,y,cex=0.5,
       xlab=paste("Days since",as.Date(t,origin = "2019-12-31")),
       ylab="Cumulative Cases", 
       main=paste0(Country, " / Logistic"),
  ...
  tryCatch(
    expr = {
      ...
      if(m>=3){
        ...
        fit31 = try(nls2(y_ ~ y_[2] + a3/(1+exp(b3-c3*x3)), start = st,
                         algorithm = "plinear-random",control = nls.control(maxiter = max_iter)))
        initial_list = data.frame(a3=coef(fit21)[1], b3=coef(fit21)[2], c3=coef(fit21)[3])
        as.list(initial_list)
        fit32 = try( nls(y_ ~ y_[2] + a3/(1+exp(b3-c3*x3)), start = initial_list) )
    ...
    error = function(e){
      ...
      if(length(coef_result[,1])>=2){
        ...
        lines(x2+(b[2]-1), pred_y, col=3,lwd=3)
      ...
      if(length(coef_result[,1])>=3){
        ...
        pred_Y = diff(pred_y)
        idx_error = (length(x2)+1) : (length(x2)+length(x3))
        MSE = MSE +
          sum( (Y[idx_error] - pred_Y)^2 ) # Mean Squared Error
        MAPE = MAPE +
          sum( abs( (Y[idx_error] - pred_y)/(Y[idx_error] + 1) ) ) # Mean Absolute Percentage Error
  ...
  tryCatch(
    expr = {
      ...
      if(m>=3){
        ...
        st = data.frame(a3 = c(0.1*max(y), max(y)*2), b3 = c(0, 25), c3 = c(0, 0.5)) 
        fit31 = try(nls2(y_ ~ y_[2] + a3*exp(-b3*exp(-c3*x3)), start = st,
                         algorithm = "plinear-random",control = nls.control(maxiter = max_iter)))
        initial_list = data.frame(a3=coef(fit31)[1], b3=coef(fit31)[2], c3=coef(fit31)[3])
        as.list(initial_list)
        fit32 = try( nls(y_ ~ y_[2] + a3*exp(-b3*exp(-c3*x3)), start = initial_list) )
