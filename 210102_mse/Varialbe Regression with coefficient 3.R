...
# Path
path <- "C:/Users/김학용/Desktop/코로나 연구 인턴/주차별 작업사항/최종 정리(날짜 수정)/"
...
df_logistic = read.csv(paste0(result_path,"segmented_Logistic_result.csv"),row.names = 1)[,1:6]
df_logistic = log(df_logistic)
df_gompertz = read.csv(paste0(result_path,"segmented_Gompertz_result.csv"),row.names = 1)[,1:6]
df_gompertz = log(df_gompertz)
df_coef=cbind(df_logistic,df_gompertz)
