...
segPoisson <- function(Country,break_point=c(),
  ...
  plot(x,y,xlab=paste("Days since",as.Date(start_date,origin = "2019-12-31")),
       ylab="Cumulative Confirmed cases",main=Country)
  cat(paste0("-----------",Country,"-----------\n"))
...
segLogistic_separate = function(Country,start_date=1,
  ...
  if(prediction_plot){
    plot(x,y,xlab=paste("Days since",as.Date(t,origin = "2019-12-31")),
         ylab="Cumulative Confirmed cases", main=paste0(Country, " / Logistic"), xlim=c(0,n+30),ylim=c(0,2*max(y))) 
  }else{
    plot(x,y,xlab=paste("Days since",as.Date(t,origin = "2019-12-31")),
         ylab="Cumulative Confirmed cases", main=paste0(Country, " / Logistic")) 
  }
  ...
  tryCatch(
    ...
    error = function(e){
      ...
      abline(v=b[-1],lty=2)
      legend("topleft",c("Real value","fit1","fit2"),pch=c(1,-1,-1),
             lty=c(0,1,1),col=c(1,2,3),cex=0.8)
      if(save_image){dev.off()}
...
segBertalanffy_separate = function(Country,start_date=1,
  ...
  if(prediction_plot){
    plot(x,y,xlab=paste("Days since",as.Date(t,origin = "2019-12-31")),
         ylab="Cumulative Confirmed cases", main=paste0(Country, " / Bertalanffy"), xlim=c(0,n+30),ylim=c(0,2*max(y))) 
  }else{
    plot(x,y,xlab=paste("Days since",as.Date(t,origin = "2019-12-31")),
         ylab="Cumulative Confirmed cases", main=paste0(Country, " / Bertalanffy")) 
  }
...
segGompertz_separate = function(Country,start_date=1,
  ...
  if(prediction_plot){
    plot(x,y,xlab=paste("Days since",as.Date(t,origin = "2019-12-31")),
         ylab="Cumulative Confirmed cases", main=paste0(Country, " / Gompertz"), xlim=c(0,n+30),ylim=c(0,2*max(y))) 
  }else{
    plot(x,y,xlab=paste("Days since",as.Date(t,origin = "2019-12-31")),
         ylab="Cumulative Confirmed cases", main=paste0(Country, " / Gompertz")) 
  }
